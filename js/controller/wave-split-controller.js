...
export default class WaveSplitController extends CanvasController {
  ...
  setPath(path) {
    ...
    this.wavePoints = path.map((p) => p);
    // Calculate fourier points, and drop the small things.
    this.fourierData = getRealFourierData(this.wavePoints).filter(
      (f) => f.amplitude > 0.001
    );
  ...
  renderWaves() {
    ...
    for (let babe = 0; babe < renderedBabies; babe++) {
      ...
      for (let i = 0; i < this.wavePoints.length; i++) {
        ...
        const sineAmt =
          waveData.amplitude *
          Math.cos(2 * Math.PI * waveData.freq * iAmt + waveData.phase);
        wave[i] = sineAmt;
